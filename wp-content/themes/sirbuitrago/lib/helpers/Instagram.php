...
class InstagramAPIClient
{
	...
	public function generate_tokens(String $code)
	{
		...
		if (is_wp_error($response)) {
			...
			return wp_mail('angelc@hellorpm.com', 'Error on the instagram API', $error_message);
		}

		$decoded = json_decode($response['body'], TRUE);

		if (!isset($decoded['access_token'])) {
			return wp_mail('angelc@hellorpm.com', 'Error on the instagram API (short lived token)', $response['body']);
		}
	...
	public function set_long_lived_token(String $shortLivedToken)
	{
		...
		if (is_wp_error($response)) {
			$error_message = $response->get_error_message();
			return wp_mail('angelc@hellorpm.com', 'WP Error on the instagram API', $error_message);
		}

		$decoded = json_decode($response['body'], TRUE);

		if (!isset($decoded['access_token'])) {
			return wp_mail('angelc@hellorpm.com', 'Error on the instagram API (long lived token)', $response['body']);
		}
...
// add_action('instagram_cron_hook', 'InstagramRPMAPI::check_token_validity');


// if (isset($_GET['code'])) {
// 	InstagramRPMAPI::generate_token(str_replace('#_', '', $_GET['code']));
// }
